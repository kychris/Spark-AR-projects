...
var box = planeTracker.child('Cube')
var box0 = planeTracker.child('Cube0')
var box1 = planeTracker.child('Cube1')
...
function initDomi(pos) {
  var pinBody = new CANNON.Body({
    mass: 0.1,
    position: pos,
    shape: new CANNON.Box(new CANNON.Vec3(2, 10, 1))
  })
...
function initPinPos() {
  return [
    ...
    new CANNON.Vec3(initialPinX + 5, initialPinY + 10, initialPinZ + -25),
    new CANNON.Vec3(initialPinX + 15, initialPinY + 10, initialPinZ + -25),
    new CANNON.Vec3(0, 200, 10),
    new CANNON.Vec3(0, 20, 20),
    new CANNON.Vec3(0, 20, 25)
  ]
}
...
worldObjects.push({ sceneObject: box, physicsObject: initBox(pinPos[10]) })
worldObjects.push({ sceneObject: box0, physicsObject: initDomi(pinPos[11]) })
worldObjects.push({ sceneObject: box1, physicsObject: initDomi(pinPos[12]) })
